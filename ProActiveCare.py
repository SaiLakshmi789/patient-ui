...
def predict_and_add_predictions(session, rf_model, data,features, target):
    ...
    def send_email(patient_id):
        print(patient_id)
        print(11)
        df_email = session.sql("select email from ETL_DB.ETL_SCH.PATIENTS where patient_id= '"+str(patient_id)+"'")
        result = df_email.collect()
        result=result[0][0]
        print(result)
        send_email=session.sql("CALL SYSTEM$SEND_EMAIL('alert_email_int','"+str(result)+"','⚠️ Health Risk Alert: Please contact your doctor — important health update','Based on your recent reports, you may be at higher health risk. We recommend contacting your Primary care Provider as soon as possible to review your results and discuss next steps. \nIf you need assistance finding a provider or scheduling an appointment, we’re here to help.Contact -1800-000-0000 \nTake care,\nProActiveCare');")
        send_email.show()
